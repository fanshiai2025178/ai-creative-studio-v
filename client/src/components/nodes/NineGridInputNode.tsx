...
export function NineGridInputNode({ data }: { data: Record<string, unknown> }) {
  ...
  // API mutations
  const generateNineGrid = trpc.storyboardWorkbench.generateNineGridStoryboard.useMutation();
  const analyzeImage = trpc.storyboardWorkbench.analyzeImageForFreezeFrame.useMutation();
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const finalPrompt = prompt.trim() || "基于参考图片生成多视角分镜";
    ...
    // 立即创建结果节点（加载状态）
    const resultNodeId = `nineGridResult-${Date.now()}`;
    ...
    try {
      ...
      const result = await generateNineGrid.mutateAsync({
  ...
  }, [referenceImageUrl, prompt, aspectRatio, generateNineGrid, nodeId, getNode, addNodes, addEdges, setNodes]);

  return (
    <Card className="w-[320px] bg-gradient-to-br from-purple-950/90 to-indigo-950/90 border-purple-500/50 shadow-lg shadow-purple-500/20">
      ...
      <CardContent className="space-y-3">
          ...
          {referenceImagePreview ? (
            <div className="relative inline-flex justify-center w-full">
              <img 
                ...
                alt="参考图片"
                className="w-auto h-auto rounded border border-purple-500/50"
                style={{ maxWidth: '100%' }}
              />
              <button
            ...
            <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-purple-500/50 rounded bg-purple-900/20">
              <Loader2 className="w-8 h-8 text-purple-400 mb-2 animate-spin" />
              <span className="text-sm text-purple-300">正在加载画布图片...</span>
            </div>
          ) : (
            <label className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-purple-500/50 rounded cursor-pointer hover:border-purple-400 transition-colors bg-purple-900/20">
              <UploadIcon className="w-8 h-8 text-purple-400 mb-2" />
              <span className="text-sm text-purple-300">点击上传参考图片</span>
              <span className="text-xs text-purple-400/70 mt-1">上传后点击「AI识别」按钮</span>
              <input
                type="file"
                accept="image/*"
                className="hidden"
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <Label className="text-purple-200 text-xs">场景描述</Label>
            {referenceImageUrl && (
              <Button
                variant="ghost"
                size="sm"
                ...
                className="h-6 px-2 text-xs text-purple-300 hover:text-purple-100 hover:bg-purple-800/50"
              >
                {isAnalyzing ? (
          ...
          <Textarea
            ...
            className="h-20 text-xs bg-purple-900/30 border-purple-500/30 text-purple-100 placeholder:text-purple-400/50 resize-none"
            disabled={isAnalyzing}
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <Select value={aspectRatio} onValueChange={(v) => setAspectRatio(v as "1:1" | "16:9" | "9:16")}>
            <SelectTrigger className="h-8 text-xs bg-purple-900/30 border-purple-500/30 text-purple-100">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <Button
          ...
          disabled={isGenerating || !referenceImageUrl || isAnalyzing}
        ...
        <p className="text-[10px] text-purple-400/70 text-center">
          上传图片后点击「AI识别」，生成 9 个不同视角的分镜
        </p>
      </CardContent>
    </Card>
  );
